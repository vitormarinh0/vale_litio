...
ggplot(df, aes(x = X._POP_Cad_x_municipio_Censo_2022, y = reorder(Município, -CENSO_2022), fill = Município)) +
  ...
  labs(title = " % da população cadastrada no Cadastro Único em idade ativa em relação à população do município em idade ativa no Censo Demográfico 2022",
...
ggplot(df_long, aes(x = Ano, y = MUNICIPIO, fill = TaxaAnalfabetismo)) +
  ...
  theme_minimal() +
  labs(title = "Taxa de Analfabetismo da população em idade ativa cadastrada no CadÚnico. Municípios do Vale do Lítio. Período: 2012-2022.",
