...
top_5_cursos_por_municipio %>% 
  ...
  gt() %>%
  tab_header(
    title = "Quantidade de Matriculas nos Cinco Maiores Cursos em Nível Superior",
    subtitle = "Municípios do Vale do Lítio - 2021"
  ) %>% 
  opt_table_font(font = google_font(name = "Karla")) %>% 
  tab_options(data_row.padding = px(6),
  ...
  tab_style(
    ...
    locations = cells_body(columns = 1) # Aplica estilo à primeira coluna
  ) %>% 
  cols_label(
    nome_curso = 'Curso/Municipio',
    total_inscritos = 'Total de Matriculas'
  ) %>% tab_source_note("Fonte: INEP. Elaboração: FJP/Diretoria de Estatística e Informações.")
...
```{r, include=FALSE}
top_5_cursos_por_municipio %>% group_by(nome_curso) %>% 
  summarise(total = sum(total_inscritos)) %>% arrange(desc(total)) %>%  gt()
```
