...
ggplot(df_long, aes(x = Ano, y = MUNICIPIO, fill = TaxaAnalfabetismo)) +
  ...
  theme_minimal() +
  labs(title = "Taxa de Analfabetismo da população em idade ativa para o trabalho (14 a 64 anos) inscrita no Cadastro Único (%)",
...
ggplot(df_long, aes(x = Ano, y = TaxaEscolarizacao, group = MUNICIPIO, color = MUNICIPIO)) +
  ...
  theme_minimal() +
  labs(title = "Taxa de Escolarização da população em idade ativa para o trabalho (14 a 64 anos) inscrita no Cadastro Único (%)",
       subtitle = "Municípios do Vale do Lítio - 2012-2022", x = "Ano", y = "Taxa de Escolarização (%)", color = "Município") +
...
ggplot(pop_idade_ativa_long, aes(x = ano, y = Percentual, group = Município, colour = Município)) +
  ...
  geom_point() +
  theme_minimal() +
  labs(title = "Distribuição da população em idade ativa para o trabalho (14 a 64 anos) inscrita no Cadastro Único, segundo o  vínculo de trabalho (%)",
...
# Criando o gráfico lollipop com transparência nas bolinhas
g <- ggplot(saldo_migratorio, aes(x = Município, y = SM)) +
  geom_segment(aes(xend = Município, yend = 0, color = SaldoTipo), size = 1.2) + # Linhas do gráfico lollipop
  geom_point(aes(fill = SaldoTipo), size = 3.5, shape = 21, position = position_dodge(width = 0.05), color = "black", alpha = 0.8) + # Pontos do gráfico lollipop com transparência
  scale_color_manual(values = custom_colors_pos_neg) +
  scale_fill_manual(values = custom_colors_pos_neg) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),
        legend.position = "top", # Posicionando a legenda no topo
        plot.title = element_text(hjust = 0.5, size = 16),
        plot.subtitle = element_text(hjust = 0.5, size = 12),
        axis.title = element_text(size = 14)) +
  labs(title = "Saldo Migratório Anual",
       subtitle = "Municípios do Vale do Lítio - 2012-2022",
       y = "Saldo Migratório",
       x = "Município") +
  guides(color = guide_legend(title = "Saldo Migratório"), fill = guide_legend(title = "Saldo Migratório"))
