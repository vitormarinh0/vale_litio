...
# Agrupar por nome do curso e somar as vagas
curso_vagas <- df %>%
  group_by(nome_curso) %>%
  summarize(quantidade_vagas = sum(quantidade_vagas, na.rm = TRUE)) %>%
  arrange(desc(quantidade_vagas))

# Pegar os 5 cursos com maior número de vagas
top_cursos <- head(curso_vagas, 5)

# Criar o gráfico
ggplot(top_cursos, aes(x=reorder(nome_curso, quantidade_vagas), y=quantidade_vagas, fill=nome_curso)) +
  geom_bar(stat="identity") +
  coord_flip() +  # Para ter barras horizontais
  theme_minimal() +
  labs(x="Curso", y="Quantidade de Vagas", title="Vagas por Curso", fill="Curso") +
  scale_fill_brewer(palette="Paired") # Usar uma paleta de cores
