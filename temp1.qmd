...
g2 <- ggplot(data_long[data_long$Município == "Vale do Lítio" & grepl("RazaoDependencia", data_long$variable), ],
             aes(x = gsub("Ano", "", variable), y = value, group = 1)) +
  geom_line(aes(color = variable), linetype = "dotted", linewidth = .8, color = "#3366FF") + # linha pontilhada com cor definida
  geom_point(aes(color = variable), linewidth = 3) +
  geom_text(aes(label = sprintf("%.1f%%", value), color = variable), vjust = 1.5, check_overlap = TRUE) +
